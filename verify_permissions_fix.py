...
def verify_role_permissions():
    ...
    for role in roles_to_test:
        ...
        if role == "admin":
            ...
            if all(p in permissions for p in expected) and all(
                p not in permissions for p in not_expected
            ):
...
def print_summary():
    ...
    print(
        "  • Operators get appropriate control permissions (read_users, remote_control)"
    )
