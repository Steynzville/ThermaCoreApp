    ...
    if not env_vars:
        ...
        if "DATABASE_URL" in env_dict:
            ...
            if "fromDatabase" in db_var:
                ...
                if (
                    from_db.get("name") == "thermacore-db"
                    and from_db.get("property") == "connectionString"
                ):
        ...
        if "SECRET_KEY" in env_dict:
            ...
            if secret_var.get("generateValue") is True:
                ...
                print(
                    "❌ FAIL: SECRET_KEY generateValue is misconfigured, expected 'generateValue: true'"
                )
                all_checks_passed = False
        else:
        ...
        if "DEBUG" in env_dict:
            ...
            if debug_var.get("value") == "False":
                ...
                print(
                    f"❌ FAIL: DEBUG value is '{debug_var.get('value')}', expected 'False'"
                )
                all_checks_passed = False
        else:
        ...
        if not databases or len(databases) == 0:
            ...
            for key, expected, description in db_checks:
                ...
                if actual == expected:
                    ...
                    print(
                        f"❌ FAIL: {description} is '{actual}', expected '{expected}'"
                    )
                    all_checks_passed = False
