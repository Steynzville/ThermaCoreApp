...
const UnitPerformance = ({ unit: propUnit, className, hideHeader = false }) => {
  ...
  const moneySavedBySelfGenerating =
    unitUserLoadKWh * financialAssumptions.electricityCost;
      ...
      <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 lg:mb-6">
              <div>
                ...
                <p className="text-sm lg:text-base text-gray-600 dark:text-gray-400 mb-2">
                  Monitor individual unit power generation, efficiency, and
        ...
        >
          <SummaryCard
            ...
            todayValue={Math.round(
              performanceData.power.today,
            ).toLocaleString()}
            monthValue={Math.round(
              performanceData.power.month,
            ).toLocaleString()}
            allTimeValue={Math.round(
              performanceData.power.allTime,
            ).toLocaleString()}
            unit="kWh"
            icon={Zap}
            color="blue"
