...
const ProtectedRoute = ({ component: Component, componentMap, roles = [], ...props }) => {
  ...
  const ComponentToRender = componentMap?.[userRole] ? componentMap[userRole] : Component;
  ...
  console.log(
    "[ProtectedRoute] Rendering protected component:",
    ComponentToRender.name || "Unknown",
  );

  return (
    <>
      <EnhancedSideNavigation userRole={userRole} />
      <ComponentToRender className={sidebarMargin} userRole={userRole} {...props} />
    </>
  );
};
