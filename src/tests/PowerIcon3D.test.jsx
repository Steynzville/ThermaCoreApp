...
import PowerIcon3D from "../components/PowerIcon3D";
...
describe("PowerIcon3D", () => {
  ...
  describe("glow effect", () => {
    it("renders glow effect when power > 0", () => {
      ...
      const glowDivs = container.querySelectorAll('.rounded-2xl');
      // Find the glow element (it has blur and opacity)
      const glowElement = Array.from(glowDivs).find(el => 
        el.classList.contains('blur-sm') && el.classList.contains('scale-110')
      );
  ...
  describe("animation effects", () => {
    it("renders pulse animation for online units", () => {
      ...
      // Ping animation dot should exist
  ...
  describe("edge cases", () => {
    it("handles negative power values", () => {
      ...
      // Should be treated as > 0 for styling (green)
      const svg = document.querySelector('svg[role="img"]');
      expect(svg).toHaveClass('text-green-500');
    });
    ...
    it("handles undefined power gracefully", () => {
      ...
      // Should default to 0 or show as power value
      // Since power is undefined, it will be treated as 0
      expect(screen.getByText("0")).toBeInTheDocument();
    ...
    it("handles null power gracefully", () => {
      ...
      expect(screen.getByText("null")).toBeInTheDocument(); // React renders null as text
