...
describe("AdminPanel User Creation Form", () => {
  ...
  beforeEach(() => {
    ...
    // Mock AuthContext
    _authSpy = vi.spyOn(AuthContext, "useAuth").mockReturnValue({
      user: mockUser,
      userRole: "admin",
    ...
    // Mock usersAPI
    _usersAPISpy = vi.spyOn(usersAPI, "getAllUsers").mockResolvedValue({
      data: mockUsers,
      page: 1,
      per_page: 100,
  ...
  it("should show error message when roles API fails", async () => {
    ...
    vi.spyOn(apiFetch, "apiGet").mockRejectedValueOnce(
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Unable to load roles. Please refresh the page."),
      ).toBeInTheDocument();
    });
