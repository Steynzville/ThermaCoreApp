...
describe("UnitControl", () => {
  ...
  it("should apply computed live metrics values when system power is on", async () => {
    // device.id = "1" -> idOffset = "1".charCodeAt(0) = 49
    // tempIn  = 100*0.4 + (49 % 5) = 40 + 4 = 44
    // tempOut = 100*0.1 + (49 % 3) = 10 + 1 = 11
    // pressure = 200*0.15 + (49 % 2) = 30 + 1 = 31
    // flowRateInlet  = 50 + (49 % 5) - 2.5 = 50 + 4 - 2.5 = 51.5
    // flowRateOutlet = 40 + (49 % 3) - 1.5 = 40 + 1 - 1.5 = 39.5
    setLocationState({
      device: { ...baseDevice, id: "1", status: "Operational" },
    });
    vi.mocked(useRealtimeMetrics).mockReturnValue({
      metrics: {
        temperature: { current: 100 },
    ...
    await waitFor(() => expect(screen.getByText("44°F")).toBeInTheDocument());
    expect(screen.getByText("11°F")).toBeInTheDocument();
    expect(screen.getByText("31 bar")).toBeInTheDocument();
    expect(screen.getByText("51.5 L/min")).toBeInTheDocument();
    expect(screen.getByText("39.5 L/min")).toBeInTheDocument();
  });

  it("should not re-trigger the live metrics effect once values stabilize", async () => {
    // Regression guard for the infinite-loop bug: render with metrics +
    // systemPower on, let it settle, and confirm the component doesn't
    // hang or keep mutating after the first stable update.
    setLocationState({
      device: { ...baseDevice, id: "1", status: "Operational" },
    });
    vi.mocked(useRealtimeMetrics).mockReturnValue({
    ...
    // If the effect were still looping, this would never resolve /
    // the test would time out. Waiting again on the same stable value
    // confirms no runaway re-renders are occurring.
    await waitFor(() => expect(screen.getByText("44°F")).toBeInTheDocument());
  });
