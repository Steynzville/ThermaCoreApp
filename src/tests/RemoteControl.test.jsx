...
describe("RemoteControl Component", () => {
  ...
  describe("Thermal & AWG Production Setpoints and Operation Modes", () => {
    it("should render setpoint sliders and operation mode buttons", () => {
      act(() => {
        render(
          <TestWrapper role="admin">
            <RemoteControl unit={mockUnit} />
      ...
      expect(screen.getByText("Thermal & AWG Production Setpoints")).toBeInTheDocument();
      expect(screen.getByText("Balanced (50/50)")).toBeInTheDocument();
      expect(screen.getByText("Power Priority (90/20)")).toBeInTheDocument();
      expect(screen.getByText("AWG Water Priority (30/90)")).toBeInTheDocument();
      expect(screen.getByText("Power Production Setpoint")).toBeInTheDocument();
      expect(screen.getByText("AWG Water Production Setpoint")).toBeInTheDocument();
    });

    it("should update setpoints when operation mode buttons are clicked", () => {
      act(() => {
        render(
          <TestWrapper role="admin">
            <RemoteControl unit={mockUnit} />
      ...
      fireEvent.click(screen.getByText("Power Priority (90/20)"));
      expect(screen.getByText("90%")).toBeInTheDocument();
      expect(screen.getByText("20%")).toBeInTheDocument();

      fireEvent.click(screen.getByText("AWG Water Priority (30/90)"));
      expect(screen.getByText("30%")).toBeInTheDocument();
      expect(screen.getByText("90%")).toBeInTheDocument();

      fireEvent.click(screen.getByText("Balanced (50/50)"));
      expect(screen.getAllByText("50%").length).toBeGreaterThan(0);
    });

    it("should update power setpoint and auto-shutdown if set to 0%", () => {
      act(() => {
        render(
          <TestWrapper role="admin">
            <RemoteControl unit={mockUnit} />
      ...
      fireEvent.change(powerSlider, { target: { value: "0" } });
      expect(screen.getByText(/Status: Stopped/i)).toBeInTheDocument();
    });

    it("should update AWG setpoint and disable water production if set to 0%", () => {
      act(() => {
        render(
          <TestWrapper role="admin">
            <RemoteControl unit={mockUnit} />
      ...
      const sliders = screen.getAllByRole("slider");
      const awgSlider = sliders[1];

      fireEvent.change(awgSlider, { target: { value: "0" } });
      expect(screen.getByText("Water production disabled via 0% AWG Setpoint")).toBeInTheDocument();
    });
  });
