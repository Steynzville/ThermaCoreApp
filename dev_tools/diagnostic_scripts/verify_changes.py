...
validation_file = os.path.join(
    os.path.dirname(__file__), "app", "utils", "validation.py"
)
...
checks = [
    (
        "validate_json_request decorator defined",
        "def validate_json_request(f):" in validation_content,
    ),
    (
        "Handles empty JSON",
        "Request must contain valid JSON data" in validation_content,
    ),
...
checks = [
    (
        "Imports validation decorator",
        "from app.utils.validation import validate_json_request" in units_content,
    ),
    (
        "create_unit uses decorator",
        "@validate_json_request\ndef create_unit():" in units_content,
    ),
    (
        "update_unit uses decorator",
        "@validate_json_request\ndef update_unit(" in units_content,
    ),
    (
        "create_unit_sensor uses decorator",
        "@validate_json_request\ndef create_unit_sensor(" in units_content,
    ),
    (
        "update_unit_status uses decorator",
...
if old_pattern_count > 0:
    print(
        f"  ✗ Found {old_pattern_count} instances of old validation pattern (should be 0)"
    )
    sys.exit(1)
else:
...
if old_badrequest_handling > 0:
    print(
        f"  ⚠ Warning: Found {old_badrequest_handling} BadRequest exception handlers (decorator handles these)"
    )
else:
