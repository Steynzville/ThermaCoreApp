...
def parse_timestamp(timestamp_str: str) -> datetime:
    ...
    try:
        ...
        if parsed_dt.tzinfo is None:
            import logging  # noqa: PLC0415 - Conditional import

            logger = logging.getLogger(__name__)
            logger.debug(
                f"Converting naive datetime '{timestamp_str}' to UTC timezone. "
...
def get_recent_sensor_readings(
    ...
    """
    from app.models import Sensor  # noqa: PLC0415 - Avoid circular import
...
def calculate_unit_efficiency(unit_id: str, hours_back: int = 24) -> dict[str, float]:
    ...
    """
    from app.models import Unit  # noqa: PLC0415 - Avoid circular import
    ...
    if water_readings:
        ...
        if len(water_values) > 1:
            ...
            metrics["water_generation_rate"] = (
                water_diff / time_diff if time_diff > 0 else 0
            )
    ...
    if metrics["average_power"] > 0 and metrics["water_generation_rate"] > 0:
        metrics["efficiency_ratio"] = (
            metrics["water_generation_rate"] / metrics["average_power"]
        )
...
def generate_health_score(unit_id: str) -> dict[str, Any]:
    ...
    if unit.last_maintenance:
        ...
        if maintenance_datetime.tzinfo is None:
            ...
            logger = logging.getLogger(__name__)
            logger.warning(
                f"Naive datetime detected for unit {unit_id} last_maintenance: {maintenance_datetime}. "
                f"Converting to UTC for calculation.",
            )
            maintenance_datetime = maintenance_datetime.replace(tzinfo=timezone.utc)

        days_since_maintenance = (
            datetime.now(timezone.utc) - maintenance_datetime
        ).days
