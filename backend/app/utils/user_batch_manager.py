...
class UserBatchManager:
    ...
    def batch_update_company(user_ids: List[int], company_name: str):
        ...
        count = User.query.filter(User.id.in_(user_ids)).update(
            {"company": company_name}, synchronize_session=False
        )
        db.session.commit()
        return count
    ...
    @staticmethod
    def get_users_by_department(department: str, company: Optional[str] = None):
    ...
    def count_users_by_company():
        ...
        return {company: count for company, count in counts}
