...
def _check_environment_mismatch(
    flask_env: str, app_env: str, debug_enabled: bool, is_testing: bool = False
) -> bool:
    ...
    if config:
        ...
        if debug_enabled:
            ...
            if not flask_env and not app_env and not app_flask_env:
                ...
                flask_env not in production_envs
                and app_env not in production_envs
                and app_flask_env not in production_envs
            ):
                return True
...
def handle_environment_detection_error(
    ...
    try:
        ...
        error_msg = (
            f"Environment detection failed during {service_name} {context}: {env_error}"
        )
        logger.error(error_msg, exc_info=True)
        runtime_error = RuntimeError(
            f"{service_name} {context} failed due to environment configuration error: {env_error}"
        )
    ...
    try:
        if is_testing_environment(app):
            error_msg = (
                f"{service_name} initialization failed in testing: {original_error}"
            )
            logger.error(error_msg, exc_info=True)
            runtime_error = RuntimeError(error_msg)
            runtime_error.__cause__ = original_error
    ...
    if isinstance(original_error, (ValueError, RuntimeError)):
        ...
        logger.error(
            f"{service_name} failed with configuration error (development). "
            f"This should be addressed: {original_error}"
        )
        return False, None
    else:
        logger.warning(
            f"{service_name} {context} failed (development): {original_error}"
        )
