...
class ServiceManager:
    ...
    def register_service(
        ...
        self._logger.debug(f"Registered service '{name}' as {service_type.value}, enabled={enabled}")
    ...
    def set_service_instance(self, name: str, instance: Any):
        ...
        if name not in self._services:
            self._logger.warning(f"Attempted to set instance for unregistered service: {name}")
            return
    ...
    def set_service_error(self, name: str, error: Exception):
        ...
        """
        if name not in self._services:
            self._logger.warning(f"Attempted to set error for unregistered service: {name}")
            return
    ...
    def get_overall_health(self) -> str:
        ...
        for name, service in self._services.items():
            ...
            if service['status'] not in [ServiceStatus.HEALTHY, ServiceStatus.NOT_INITIALIZED]:
...
def initialize_service(
    ...
    **kwargs
    ...
    if manager_name not in service_manager._services:
