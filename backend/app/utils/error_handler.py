...
class SecurityAwareErrorHandler:
    ...
    GENERIC_MESSAGES: ClassVar[dict[str, str]] = {
    ...
    def handle_thermacore_exception(exception) -> tuple[Any, int]:
        ...
        log_message = f"Domain exception [{request_id}] {exception.__class__.__name__} in {exception.context}: {exception!s}"
    ...
    def handle_service_error(
        ...
        log_message = f"Service error [{request_id}] {error.__class__.__name__} in {context}: {error!s}"
    ...
    def handle_permission_error(
        error: Exception,
        context: str = "Permission check",
    ) -> tuple[Any, int]:
        """Handle permission errors."""
        return SecurityAwareErrorHandler.handle_service_error(
            error,
            "permission_error",
            f"Permission {context}",
            403,
        )
    ...
    def handle_value_error(
        ...
        log_message = f"ValueError [{request_id}] in {context}: {error!s}"
        logger.error(
            log_message,
            exc_info=True,
            extra={
