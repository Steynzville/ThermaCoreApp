...
def is_heartbeat_stale(
    last_heartbeat: Optional[datetime], heartbeat_timeout_seconds: int = 300
) -> bool:
...
def compute_availability_level(
    ...
    if (
        connected
        and status == "ready"
        and not is_heartbeat_stale(last_heartbeat, heartbeat_timeout_seconds)
    ):
    ...
    if connected or status in ["ready", "degraded"]:
        ...
        if (
            is_heartbeat_stale(last_heartbeat, heartbeat_timeout_seconds)
            or status == "degraded"
        ):
            return AvailabilityLevel.DEGRADED
...
def record_error(
    error_code: str, error_message: str = None, context: Dict[str, Any] = None
) -> Dict[str, Any]:
