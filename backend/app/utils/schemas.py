    ...
    def _serialize(self, value, attr, obj, **kwargs):
        ...
        if isinstance(value, str):
            try:
                ...
                logger.warning(
                    f"Invalid datetime string '{value}' in field '{attr}': {e}"
                )
...
class UnitSchema(SQLAlchemyAutoSchema):
    ...
    sensors = fields.Nested(
        "SensorSchema", many=True, dump_only=True, exclude=("unit",)
    )
...
class SensorCreateSchema(Schema):
    ...
    unit_id = fields.Str(
        required=False
    )  # Optional since it's overridden from path parameter
    name = fields.Str(required=True, validate=validate.Length(min=1, max=100))
    sensor_type = fields.Str(required=True, validate=validate.Length(min=1, max=50))
    unit_of_measurement = fields.Str(validate=validate.Length(max=20))
...
class SensorReadingCreateSchema(Schema):
    ...
    quality = fields.Str(
        load_default="GOOD", validate=validate.OneOf(["GOOD", "BAD", "UNCERTAIN"])
    )
...
class HistoricalDataQuerySchema(Schema):
    ...
    limit = fields.Int(
        required=False, validate=validate.Range(min=1, max=10000), load_default=1000
    )
    ...
    def validate_date_range(self, data, **kwargs):
        ...
        if start_date and end_date and start_date >= end_date:
            raise ValidationError(
                "start_date must be before end_date", field_name="start_date"
            )
...
class StatisticsQuerySchema(Schema):
    ...
    days = fields.Int(
        required=False, validate=validate.Range(min=1, max=365), load_default=30
    )
...
class TrendsQuerySchema(Schema):
    ...
    days = fields.Int(
        required=False, validate=validate.Range(min=1, max=365), load_default=7
    )
    sensor_type = fields.Str(required=False)
...
class ExportDataQuerySchema(Schema):
    ...
    format = fields.Str(
        required=False, validate=validate.OneOf(["json", "csv"]), load_default="json"
    )
    start_date = fields.DateTime(required=False, format="iso")
    end_date = fields.DateTime(required=False, format="iso")
    sensor_types = fields.Str(required=False)
