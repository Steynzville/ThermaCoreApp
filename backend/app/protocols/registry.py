...
# Registry maps protocol names to their corresponding Flask app attributes
...
def collect_protocol_status() -> list[dict]:
    ...
    for name, attr in REGISTRY:
        ...
        if name in processed_names:
            logger.warning(
                f"Duplicate protocol name '{name}' encountered during collection",
            )
            continue
                ...
                if isinstance(raw, dict):
                    ...
                    if raw.get("last_heartbeat"):
                        try:
                            ...
                            logger.warning(
                                f"Invalid heartbeat format for protocol '{name}': {raw.get('last_heartbeat')}",
                            )
                    ...
                    status_obj = ProtocolStatus(
                        ...
                        heartbeat_timeout_seconds=raw.get(
                    ...
                    if raw.get("last_error_time"):
                        try:
                            if isinstance(raw["last_error_time"], str):
                                status_obj.last_error_time = datetime.fromisoformat(
                                    raw["last_error_time"].replace("Z", "+00:00"),
                                )
                            elif isinstance(raw["last_error_time"], datetime):
                                status_obj.last_error_time = raw["last_error_time"]
                        except (ValueError, TypeError):
                            logger.warning(
                                f"Invalid last_error_time format for protocol '{name}': {raw.get('last_error_time')}",
                            )
