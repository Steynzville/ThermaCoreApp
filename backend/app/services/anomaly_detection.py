...
class AnomalyDetectionService:
    ...
    def analyze_sensor_reading(
        self, sensor_id: str, unit_id: str, value: float, timestamp: datetime = None
    ) -> AnomalyResult:
        ...
        try:
            ...
            z_anomaly, z_score, z_stats = (
                self._statistical_detector.detect_z_score_anomalies(
            ...
            for reading, sensor in recent_readings:
                ...
                if result.is_anomaly:
                    ...
                    stats["avg_anomaly_score"] = (
                        stats["avg_anomaly_score"] * (stats["anomaly_count"] - 1)
                        + result.anomaly_score
                    ) / stats["anomaly_count"]
                    stats["max_anomaly_score"] = max(
                        stats["max_anomaly_score"], result.anomaly_score
                    )
    ...
    def _get_historical_values(self, sensor_id: str, days: int = 7) -> List[float]:
        ...
        readings = (
            db.session.query(SensorReading.value)
            .filter(
                and_(
