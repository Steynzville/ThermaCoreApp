...
class MockDNP3Master:
    ...
    def read_bulk_data(
        self, outstation_address: int, data_groups: Dict[str, Tuple[int, int]]
        ...
        for data_type, (start_index, count) in data_groups.items():
            if data_type == "binary_inputs":
                results[data_type] = self.read_binary_inputs(
                    outstation_address, start_index, count, _is_bulk=True
                )
    ...
    def write_binary_output(
        self, outstation_address: int, index: int, value: bool
    ) -> bool:
    ...
    def write_analog_output(
        self, outstation_address: int, index: int, value: float
    ) -> bool:
...
class DNP3Service:
        ...
        try:
            if device_id in self._devices:
                ...
                with self._cache_lock:
                    keys_to_remove = [
                        key for key in self._data_cache.keys() if key[0] == device_id
                    ]
            ...
            for point_config in data_points:
                data_point = DNP3DataPoint(
                    ...
                    description=point_config.get(
                        "description", f"Point {point_config['index']}"
                    ),
                ...
                if self._enable_bulk_operations and hasattr(
                    ...
                    try:
                        ...
                        bulk_results = self._master.read_bulk_data(
                            device.outstation_address, data_groups
                        )
    ...
    def write_data_point(
        self, device_id: str, index: int, data_type: str, value: Any
    ) -> bool:
    ...
    def enable_performance_optimizations(
        self, caching: bool = True, bulk_operations: bool = True
    ):
