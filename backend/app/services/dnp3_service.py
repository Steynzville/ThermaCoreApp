...
# This is used for simulation data, not security-critical operations
    ...
    def get_operation_stats(self, operation: str) -> dict[str, Any]:
        ...
        with self.lock:
            ...
            stats = {
                ...
                "success_rate": (
                    self.operation_counts[operation] - self.error_counts[operation]
                )
...
class MockDNP3Master:
    ...
    def __init__(self, master_address: int = 1):
        ...
        self.performance_metrics = None  # Set by parent service
    ...
    def connect_outstation(self, outstation_address: int) -> bool:
        ...
        try:
            ...
            connection_delay = secure_random.uniform(0.1, 0.3)  # Simulate network delay
            ...
            logger.error(
                f"Failed to connect to DNP3 outstation {outstation_address}: {e}",
            )
            return False
    ...
    def disconnect_outstation(self, outstation_address: int) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to disconnect from DNP3 outstation {outstation_address}: {e}",
            )
            return False
    ...
    def read_binary_inputs(
        ...
        count: int,
        _is_bulk: bool = False,
    ) -> list[tuple[int, bool, DNP3Quality]]:
        ...
        if not _is_bulk:
            network_delay = min(
                0.05 * (count / 10),
                0.5,
            )  # Scale with data size, cap at 0.5s
        ...
        for i in range(count):
            ...
            if cache_key in self.data_cache[outstation_address]:
                ...
                value = prev_value if secure_random.random() > 0.1 else not prev_value
            else:
                value = secure_random.choice([True, False])
    ...
    def read_analog_inputs(
        ...
        count: int,
        _is_bulk: bool = False,
    ) -> list[tuple[int, float, DNP3Quality]]:
        ...
        network_delay = min(
            0.08 * (count / 10),
            0.8,
        )  # Analog reads are slightly slower
        time.sleep(network_delay)
        ...
        for i in range(count):
            ...
            if cache_key in self.data_cache[outstation_address]:
                ...
                variation = prev_value * 0.05 * (secure_random.random() - 0.5)
                value = max(0, prev_value + variation)
            # Generate realistic analog values based on index
            elif index < 10:  # Temperature sensors
                value = 20.0 + secure_random.uniform(0, 60)  # 20-80°C
            elif index < 20:  # Pressure sensors
                value = secure_random.uniform(0, 150)  # 0-150 PSI
            elif index < 30:  # Flow sensors
                value = secure_random.uniform(0, 100)  # 0-100 L/min
            else:
                value = secure_random.uniform(0, 1000)  # Generic 0-1000
    ...
    def read_counters(
        ...
        count: int,
        _is_bulk: bool = False,
    ) -> list[tuple[int, int, DNP3Quality]]:
        ...
        for i in range(count):
            ...
            if cache_key in self.data_cache[outstation_address]:
                prev_value = self.data_cache[outstation_address][cache_key]
                value = prev_value + secure_random.randint(0, 10)  # Increment
            else:
                value = secure_random.randint(0, 1000000)  # Initial counter value
    ...
    def read_bulk_data(
        self,
        outstation_address: int,
        data_groups: dict[str, tuple[int, int]],
    ) -> dict[str, list[tuple[int, Any, DNP3Quality]]]:
        ...
        for data_type, (start_index, count) in data_groups.items():
            if data_type == "binary_inputs":
                results[data_type] = self.read_binary_inputs(
                    outstation_address,
                    start_index,
                    count,
                    _is_bulk=True,
                ...
                results[data_type] = self.read_analog_inputs(
                    outstation_address,
                    start_index,
                    count,
                    _is_bulk=True,
                ...
                results[data_type] = self.read_counters(
                    outstation_address,
                    start_index,
                    count,
                    _is_bulk=True,
        ...
        logger.info(
            f"Bulk read {total_points} data points from outstation {outstation_address}",
        )
        return results
    ...
    def write_binary_output(
        ...
        logger.info(
            f"Writing binary output {index} = {value} to outstation {outstation_address}",
        )
    ...
    def write_analog_output(
        ...
        logger.info(
            f"Writing analog output {index} = {value} to outstation {outstation_address}",
        )
        # Simulate write operation
        return True
...
class DNP3Service:
    ...
    def __init__(self, app=None):
        ...
        self._master: MockDNP3Master | None = None
        ...
        self._cache_cleanup_interval = (
            300  # Kept for compatibility, but not actively used
        )
    ...
    def performance_metrics(self):
        """Get performance metrics (public accessor for _performance_metrics)."""
    ...
    def init_master(self, master_address: int = 1):
        ...
        self._master.performance_metrics = self.performance_metrics
    ...
    def add_device(
        ...
        try:
            ...
            logger.info(
                f"Added DNP3 device: {device_id} (outstation {outstation_address} at {host}:{port})",
            )
            return True
        ...
        try:
            if device_id in self._devices:
                ...
                with self._cache_lock:
                    keys_to_remove = [
                        key for key in self._data_cache if key[0] == device_id
                    ]
        ...
        try:
            ...
            for point_config in data_points:
                data_point = DNP3DataPoint(
                    ...
                    description=point_config.get(
                        "description",
                        f"Point {point_config['index']}",
                    ),
            ...
            logger.info(
                f"Added {len(point_configs)} data point configs for device {device_id}",
            )
            return True
    ...
    def read_device_data(self, device_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if self._enable_caching:
                ...
                if cached_readings:
                    logger.debug(
                        f"Using {len(cached_readings)} cached readings for device {device_id}",
                    )
            ...
            if data_points:  # Only read if there are uncached data points
                ...
                binary_inputs = [
                    dp
                    for dp in data_points
                    if dp.data_type == DNP3DataType.BINARY_INPUT
                ]
                analog_inputs = [
                    dp
                    for dp in data_points
                ...
                counters = [
                    dp for dp in data_points if dp.data_type == DNP3DataType.COUNTER
                ]

                # Use bulk operations if enabled and available
                if self._enable_bulk_operations and hasattr(
                    self._master,
                    "read_bulk_data",
                ):
                    try:
                        ...
                        if binary_inputs:
                            ...
                            data_groups["binary_inputs"] = (
                                min(indices),
                                max(indices) - min(indices) + 1,
                            )
                        if analog_inputs:
                            indices = [dp.index for dp in analog_inputs]
                            data_groups["analog_inputs"] = (
                                min(indices),
                                max(indices) - min(indices) + 1,
                            )
                        if counters:
                            indices = [dp.index for dp in counters]
                            data_groups["counters"] = (
                                min(indices),
                                max(indices) - min(indices) + 1,
                            )

                        # Single bulk read operation
                        bulk_results = self._master.read_bulk_data(
                            device.outstation_address,
                            data_groups,
                        )
                        ...
                        logger.error(
                            f"Bulk operations failed for device {device_id}: {e}",
                        )
                    ...
                    if binary_inputs:
                        ...
                        for index, value, quality in binary_readings:
                            ...
                            if index in binary_lookup:
                                ...
                                logger.warning(
                                    f"Received binary input for unconfigured index {index} from device {device_id}",
                                )
                    ...
                    if analog_inputs:
                        ...
                        for index, raw_value, quality in analog_readings:
                            ...
                            if index in analog_lookup:
                                ...
                                # Log when received index is not in our configuration
                                logger.warning(
                                    f"Received analog input for unconfigured index {index} from device {device_id}",
                                )
                    ...
                    if counters:
                        ...
                        for index, value, quality in counter_readings:
                            ...
                            if index in counter_lookup:
                                ...
                                # Log when received index is not in our configuration
                                logger.warning(
                                    f"Received counter for unconfigured index {index} from device {device_id}",
                                )
                ...
                if self._enable_caching and readings:
                    ...
                    new_readings = [
                        r for r in readings if r.index not in cached_indices
                    ]
                    if new_readings:
                        with self._cache_lock:
    ...
    def write_data_point(
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Successfully wrote {value} to {data_type} index {index} on device {device_id}",
                )
    ...
    def _cleanup_cache_if_needed(self):
        ...
        now = utc_now()
        if (
            now - self._last_cache_cleanup
        ).total_seconds() > self._cache_cleanup_interval:
            self._last_cache_cleanup = now
            # Log current cache sizes for monitoring
            logger.debug(
                f"Cache status - Connections: {len(self._connection_pool)}, "
        ...
        for device_id, device in self._devices.items():
            if hasattr(device, "is_connected") and device.is_connected:
                device_stats[device_id] = {
                    ...
                    "outstation_address": device.outstation_address,
                    "data_points_configured": len(
                        self._data_point_configs.get(device_id, []),
                    ),
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        total_ops = sum(
            stat.get("count", 0) for stat in metrics.values() if "count" in stat
        )
        avg_response_time = 0
        total_errors = 0
        ...
        if metrics:
            ...
            avg_response_time = (
                total_weighted_time / total_weight if total_weight > 0 else 0
            )
            total_errors = sum(
                stat.get("errors", 0) for stat in metrics.values() if "errors" in stat
            )

        success_rate = (
            ((total_ops - total_errors) / total_ops * 100) if total_ops > 0 else 100
        )
        ...
        return {
            ...
            "active_devices": len(
                [d for d in self._devices.values() if d.is_connected],
            ),
            "cached_readings": len(self._data_cache),
    ...
    def enable_performance_optimizations(
        ...
        logger.info(
            f"DNP3 performance optimizations updated: caching={caching}, bulk_ops={bulk_operations}",
        )
    ...
    def clear_performance_metrics(self):
        ...
        self._performance_metrics = DNP3PerformanceMetrics()
        if self._master:
            self._master.performance_metrics = self._performance_metrics
    ...
    def get_device_status(self, device_id: str | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "connected_devices": sum(
                1 for d in self._devices.values() if d.is_connected
            ),
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        )  # Service available if master is initialized
        ...
        metrics = {
            ...
            "total_data_points": sum(
                len(configs) for configs in self._data_point_configs.values()
            ),
        ...
        is_demo_mode = (
            ...
            or is_testing
            or os.getenv("DNP3_DEMO", "false").lower() == "true"
        )
