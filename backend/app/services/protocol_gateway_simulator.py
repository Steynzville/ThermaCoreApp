...
class ProtocolGatewaySimulator:
    ...
    def __init__(self, mqtt_broker_host="localhost", mqtt_broker_port=1883):
        ...
        self.simulation_units = ["UNIT001", "UNIT002", "UNIT003", "UNIT004", "UNIT005"]
    ...
    def init_unit_states(self):
        ...
        for unit_id in self.simulation_units:
            ...
            for sensor_type, config in self.sensor_configs.items():
                base_val = config["base_value"] + secure_random.uniform(
                    -config["noise_factor"],
                    config["noise_factor"],
                )
                self.unit_states[unit_id]["last_values"][sensor_type] = base_val
                self.unit_states[unit_id]["trend_direction"][sensor_type] = (
                    secure_random.choice([-1, 1])
                )
    ...
    def generate_sensor_value(self, unit_id: str, sensor_type: str) -> dict[str, Any]:
        ...
        base_change = secure_random.uniform(0.1, 2.0) * trend
        ...
        if secure_random.random() < 0.1:  # 10% chance to change trend
            unit_state["trend_direction"][sensor_type] *= -1

        # Check for anomalies
        quality = "GOOD"
        if secure_random.random() < unit_state["anomaly_probability"]:
            # Generate anomalous reading
            if secure_random.random() < 0.5:
                new_value = secure_random.uniform(
                    min_val * 0.5,
                    max_val * 1.2,
                )  # Out of normal range
        ...
        if secure_random.random() < 0.01:  # 1% chance
            quality = "BAD"
    ...
    def publish_sensor_data(self, unit_id: str, sensor_type: str):
        ...
        try:
            ...
            if result.rc == mqtt.MQTT_ERR_SUCCESS:
                logger.debug(
                    f"Published {unit_id}/{sensor_type}: {sensor_data['value']}",
                )
            else:
    ...
    def simulate_unit_status_change(self, unit_id: str):
        ...
        """
        if secure_random.random() < 0.001:  # Very rare status changes
            current_status = self.unit_states[unit_id]["status"]

            # Possible status transitions
            if current_status == "online":
                new_status = secure_random.choice(["maintenance", "warning"])
            elif current_status in ["maintenance", "warning"]:
                new_status = "online" if secure_random.random() < 0.7 else "offline"
    ...
    def inject_test_scenario(self, scenario_type: str, unit_id: str | None = None):
        ...
        """
        if not unit_id:
            unit_id = secure_random.choice(self.simulation_units)
        ...
        if scenario_type == "high_temperature":
            ...
            sensor_type = secure_random.choice(list(self.sensor_configs.keys()))
            sensor_data = {
                "value": 0.0,
                "quality": "BAD",
                "timestamp": datetime.now(timezone.utc).isoformat(),
