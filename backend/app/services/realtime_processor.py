...
class RealTimeDataProcessor:
    ...
    def process_sensor_data(self, unit_id: str, sensor_type: str, data: Dict[str, Any]):
        ...
        try:
            ...
            websocket_service.broadcast_sensor_data(
                unit_id, sensor_type, processed_data
            )
    ...
    def _validate_and_transform_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if processed["quality"] not in valid_qualities:
            logger.warning(
                f"Invalid quality value: {processed['quality']}, setting to UNCERTAIN"
            )
    ...
    def _check_alert_rules(
        self, unit_id: str, sensor_type: str, data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def process_unit_status_change(
        self, unit_id: str, old_status: str, new_status: str
    ):
    ...
    def process_device_status_change(self, device_id: str, status_change: dict):
        ...
        """
        logger.info(
            f"Device {device_id} status changed: {len(status_change.get('changes', []))} changes detected"
        )

        # Broadcast device status update to subscribed clients
        websocket_service.broadcast_device_status(
            device_id,
            {
                "device_id": device_id,
        ...
        for change in status_change.get("changes", []):
            if change.get("severity") in ["critical", "warning"]:
                alert = {
                    ...
                    "message": change.get(
                        "message", f"Device {device_id} status changed"
                    ),
                    "device_id": device_id,
                    "device_name": status_change.get("deviceName", device_id),
                    "timestamp": status_change.get(
                        "timestamp", datetime.now(timezone.utc)
                    ),
                }
                websocket_service.broadcast_system_alert(alert)
