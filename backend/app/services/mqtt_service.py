...
class MQTTClient:
    ...
    def __init__(self, app=None, data_storage_service=None):
        ...
        self._subscribed_topics: set = (
            set()
        )  # Use set for efficient unique topic management
    ...
    def init_app(self, app, data_storage_service=None):
        ...
        try:
            ...
            logger.info(
                f"MQTT configuration loaded - Broker: {self.broker_host}:{self.broker_port}, TLS: {self.use_tls}"
            )
        ...
        try:
            ...
            logger.info(
                f"MQTT Certificate paths - CA: {self.ca_certs}, Cert: {self.cert_file}, Key: {self.key_file}"
            )
        ...
        try:
            self.default_topics = app.config.get(
                "MQTT_SCADA_TOPICS",
                [
                    "scada/+/temperature",
                    "scada/+/pressure",
            ...
            if success:
                ...
                try:
                    ...
                    realtime_processor.process_sensor_data(
                        data["unit_id"], data["sensor_type"], data
                    )
                except ImportError:
                    logger.warning("Real-time processor not available")

                logger.debug(
                    f"Successfully stored and processed sensor data: {data['unit_id']}/{data['sensor_type']}"
                )
            else:
                logger.error(
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "subscribed_topics": list(
                sorted(self._subscribed_topics)
