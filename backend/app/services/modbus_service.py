...
class ModbusClient:
    ...
    def read_holding_registers(
        ...
        for i in range(count):
            ...
            if reg_addr < 100:  # Temperature registers
                ...
                value = int(
                    (0 + random.uniform(0, 150)) * 100
                )  # 0-150 PSI scaled by 100
            elif reg_addr < 300:  # Flow registers
                value = int(
                    (0 + random.uniform(0, 100)) * 100
                )  # 0-100 L/min scaled by 100
    ...
    def write_single_register(self, address: int, value: int, unit_id: int = 1) -> bool:
        ...
        if _is_sensitive_logging_enabled():
            logger.info(
                f"Writing register at address {address} to device (unit {unit_id})"
            )
        else:
    ...
    def write_multiple_registers(
        self, address: int, values: List[int], unit_id: int = 1
    ) -> bool:
        ...
        if _is_sensitive_logging_enabled():
            logger.info(
                f"Writing {len(values)} register(s) at address {address} to device (unit {unit_id})"
            )
        else:
...
class ModbusService:
    ...
    def _log_register_read(
        ...
        if _is_sensitive_logging_enabled():
            ...
            logger.debug(
                f"Reading {count} {register_type}(s) at address {address} "
            ...
            logger.debug(
                f"Reading {count} {register_type}(s) from device {device_id}{context}"
            )

    def _log_register_read_error(
        self,
        device_id: str,
        ...
        logger.error(
            f"Failed to read register on device {device_id}{context}: {error_msg}"
        )

    def _log_register_write(
        self,
        device_id: str,
        register_type: str,
        address: int = None,
        ...
        if _is_sensitive_logging_enabled():
            ...
            logger.info(
                f"Writing {operation}{addr_info}{value_info}{type_info} "
    ...
    def _log_register_write_success(
        self,
        device_id: str,
        register_type: str,
        address: int = None,
        ...
        if _is_sensitive_logging_enabled():
            ...
            addr_info = f" at address {address}" if address is not None else ""
            logger.info(
                f"Successfully wrote {operation}{addr_info} to {register_type} "
    ...
    def _log_register_write_error(
        ...
        context = f" ({', '.join(context_parts)})" if context_parts else ""
        logger.error(
            f"Failed to write register on device {device_id}{context}: {error_msg}"
        )
    ...
    def add_register_config(
        self, device_id: str, registers: List[Dict[str, Any]]
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Added {len(register_configs)} register configs for device {device_id}"
            )
            return True
            ...
            for register in register_configs:
                try:
                    ...
                    if register.register_type == "holding_register":
                        ...
                        logger.warning(
                            f"Unknown register type: {register.register_type}"
                        )
                        continue
    ...
    def write_register(
        ...
        try:
            ...
            if register_type == "coil":
                ...
                self._log_register_write(
                    device_id, register_type, address, value, data_type
                )
                ...
                if data_type == "float32":
                    ...
                    combined = struct.unpack(">I", bytes_val)[
                        0
                    ...
                    success = client.write_multiple_registers(
                        address, [high_word, low_word], device.unit_id
                    )
                else:
                    ...
                    int_value = int(value)
                    success = client.write_single_register(
                        address, int_value, device.unit_id
                    )
            else:
            ...
            if success:
                ...
                is_multiple = (
                    register_type == "holding_register" and data_type == "float32"
                )
                self._log_register_write_success(
                    device_id, register_type, address, is_multiple
                )
        ...
        if device_id:
            ...
            for dev_id, device in self._devices.items():
                devices_status[dev_id] = {
                    ...
                    "last_reading": self._last_readings.get(dev_id, {}).get(
                        "timestamp"
                    ),
                }
    ...
    def _process_register_value(
        self, raw_values: List, data_type: str, scale_factor: float, offset: float
    ) -> float:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "total_registers": sum(
                len(configs) for configs in self._register_configs.values()
            ),
        ...
        is_demo_mode = (
            ...
            or is_testing
            or os.getenv("MODBUS_DEMO", "false").lower() == "true"
        )
