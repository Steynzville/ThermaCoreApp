...
class ModbusClient:
    ...
    def read_coils(self, address: int, count: int, _unit_id: int = 1) -> list[bool]:
        """Read coil status."""
        if not self.connected:
            raise ConnectionError("Not connected to Modbus device")

        # Simulate coil readings
        coils = [secure_random.choice([True, False]) for _ in range(count)]
        # Use configurable logging for register addresses
        if _is_sensitive_logging_enabled():
        ...
        try:
            ...
            if register_type == "coil":
                ...
                if data_type == "float32":
                    ...
                    import struct  # noqa: PLC0415 - Conditional import
