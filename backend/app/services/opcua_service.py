...
class OPCUAClient:
    ...
    VALID_SECURITY_POLICIES: ClassVar[dict[str, str]] = {
        "None": "insecure",
        "Basic128Rsa15": "weak",
        "Basic256": "weak",
    ...
    POLICIES_WITHOUT_CERTIFICATES: ClassVar[list[str]] = ["None"]

    # Security limits to prevent DoS attacks (CVE-related)
    MAX_DATA_SIZE = 1024 * 1024  # 1MB max per node value
    MAX_NODES_PER_POLL = 100  # Limit concurrent node reads
    RATE_LIMIT_WINDOW = 1.0  # seconds between polls
    ...
    def __init__(self, app=None, data_storage_service=None):
        ...
        self._node_mappings: dict[
            str,
            dict[str, str],
        ] = {}  # Map node IDs to unit/sensor info
        self._last_poll_time = 0  # Rate limiting for polls
    ...
    def _validate_security_policy(
        ...
        if policy not in self.VALID_SECURITY_POLICIES:
            raise ValueError(
                f"Invalid OPC UA security policy: {policy}. "
        ...
        if require_strong and policy_strength != "strong":
            raise ValueError(
                f"OPC UA security policy '{policy}' is too weak for production. "
    ...
    def _normalize_certificate_datetime(self, dt):
        ...
        if isinstance(dt, str):
            try:
                ...
                if parsed_dt.tzinfo is None:
                    logger.debug(
                        f"Certificate datetime '{dt}' parsed as naive - assuming UTC timezone as per X.509 standard",
                    )
        ...
        if hasattr(dt, "year"):  # It's a datetime-like object
            logger.debug(
                "Certificate datetime object is naive - assuming UTC timezone as per X.509 standard",
            )
    ...
    def _load_trust_certificate(self, is_prod: bool):
        ...
        if not self.trust_cert_file:
            if is_prod:
                logger.error(
                    "OPC UA server certificate trust not configured - this is required for production security",
                )
                raise ValueError(
                    "OPC UA server certificate trust must be configured in production environment",
                )
            return
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.exception("Invalid certificate format detected")
            ...
            try:
                ...
                if not_valid_before is None:
                    raise AttributeError(
                        "Certificate has no not_valid_before attribute",
                    )

                not_valid_after_utc = self._normalize_certificate_datetime(
                    not_valid_after,
                )
                not_valid_before_utc = self._normalize_certificate_datetime(
                    not_valid_before,
                )
            ...
            if not_valid_after_utc < now:
                logger.error(
                    "Certificate validation failed: expired certificate detected",
                    exc_info=True,
                )
                raise ValueError(
                    f"Server certificate has expired: {not_valid_after_utc}",
                )
            if not_valid_before_utc > now:
                ...
                raise ValueError(
                    f"Server certificate is not yet valid: {not_valid_before_utc}",
                )
            ...
            try:
                ...
                sanitized_path = "***" if is_prod else self.trust_cert_file
                logger.info(
                    f"OPC UA server certificate loaded and trusted from: {sanitized_path}",
                )
            except Exception as e:
                # If the opcua library doesn't support this method, fall back to manual trust store management
                logger.warning(
                    f"Could not load server certificate via opcua library: {e}",
                )
                sanitized_path = "***" if is_prod else self.trust_cert_file
                logger.info(
                    f"OPC UA server certificate validated and configured for trust from: {sanitized_path}",
                )

        except OSError as e:
            # File I/O errors - log with sanitized path and handle based on environment
            logger.exception("Failed to read OPC UA trust certificate file")
            if is_prod:
            ...
            logger.warning(
                f"OPC UA trust certificate file I/O error (development): {e}",
            )
            ...
            logger.exception(
                "Unexpected error during OPC UA trust certificate loading",
            )
            if is_prod:
                raise ValueError(
                    f"Failed to load OPC UA trust certificate in production: {e}",
                ) from e
            # Only allow connection/loading issues in development
            logger.warning(
                f"OPC UA trust certificate loading failed (development): {e}",
            )
        ...
        try:
            ...
            if is_prod:
                if not self.username or not self.password:
                    logger.error(
                        "OPC UA authentication not configured - this is not allowed in production",
                    )
                    logger.warning(
                        f"OPC UA service cannot initialize without authentication in production - {SERVICE_UNAVAILABLE_MSG}",
                    )
                    return  # Gracefully return without initializing

                if self.security_policy == "None" or self.security_mode == "None":
                    logger.error(
                        "OPC UA security policy/mode set to None - this is not allowed in production",
                    )
                    logger.warning(
                        f"OPC UA service cannot initialize without security in production - {SERVICE_UNAVAILABLE_MSG}",
                    )
                    return  # Gracefully return without initializing
            ...
            if self.security_policy != "None" and self.security_mode != "None":
                try:
                    ...
                    if not (self.cert_file and self.private_key_file):
                        ...
                        if (
                            ...
                            not in self.POLICIES_WITHOUT_CERTIFICATES
                        ):
                            if is_prod:
                                # In production, require certificates for non-None policies
                                raise ValueError(
                                    f"OPC UA security policy '{self.security_policy}' requires client certificates in production. "
                                    f"Please configure OPCUA_CERT_FILE and OPCUA_PRIVATE_KEY_FILE.",
                                )
                            # Check explicit flag for insecure fallback in development
                            allow_fallback = app.config.get(
                                "OPCUA_ALLOW_INSECURE_FALLBACK",
                                False,
                            )
                            if not allow_fallback:
                                raise ValueError(
                                    f"OPC UA security policy '{self.security_policy}' requires client certificates. "
                    ...
                    try:
                        ...
                        if self.cert_file and self.private_key_file:
                            ...
                            if not security_string or security_string.count(",") < 3:
                                raise ValueError(
                                    f"Invalid security string format: '{security_string}'. Expected format: 'Policy,Mode,cert,key'",
                                )
                            ...
                            if not allow_fallback:
                                raise AssertionError(
                                    "Reached insecure fallback without OPCUA_ALLOW_INSECURE_FALLBACK being True. This is a bug.",
                                )
                            logger.warning(
                                f"DEVELOPMENT ONLY: Falling back to insecure OPC UA connection (no encryption/authentication) "
                        ...
                        if self.trust_cert_file:
                            # Sanitize certificate paths in log messages for security
                            sanitized_trust_path = (
                                "***" if is_prod else self.trust_cert_file
                            )
                            trust_info = (
                                f" (trusted server cert: {sanitized_trust_path})"
                            )

                        if is_prod:
                            logger.info(
                                f"OPC UA security enabled for production: {self.security_policy}, {self.security_mode}{cert_info}{trust_info}",
                            )
                        else:
                            logger.info(
                ...
                logger.error(
                    "OPC UA security policy and mode set to None - this is not allowed in production",
                )
                logger.warning(
                    f"OPC UA service cannot initialize without security in production - {SERVICE_UNAVAILABLE_MSG}",
                )
                ...
                if self.trust_cert_file:
                    self._load_trust_certificate(is_prod)
                logger.info(
                    "DEVELOPMENT ONLY: OPC UA security disabled - using insecure connection (no encryption/authentication). "
    ...
    def read_node_value(self, node_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            # Security: Validate data size to prevent DoS attacks (CVE mitigation)
            value_size = sys.getsizeof(raw_value)
            if value_size > self.MAX_DATA_SIZE:
                logger.error(
                    f"Node {node_id} value size ({value_size} bytes) exceeds maximum allowed ({self.MAX_DATA_SIZE} bytes)",
                )
                return None
    ...
    def read_all_subscribed_nodes(self) -> dict[str, dict[str, Any]]:
        ...
        for node_id in self._subscribed_nodes:
        ...
        try:
            with self._app.app_context():
                ...
                if success:
                    ...
                    logger.debug(
                        f"Successfully processed and stored OPC UA data: {processed_data['unit_id']}/{processed_data['sensor_type']}",
                    )
                else:
                    logger.error(
                        f"Failed to store OPC UA data: {processed_data['unit_id']}/{processed_data['sensor_type']}",
                    )
    ...
    def poll_subscribed_nodes(self):
        ...
        # Security: Rate limiting to prevent DoS attacks (CVE mitigation)
        current_time = time.time()
        if current_time - self._last_poll_time < self.RATE_LIMIT_WINDOW:
            logger.debug(
                "Rate limit applied - skipping poll (last poll %.2fs ago)",
                current_time - self._last_poll_time,
            )
            return
        self._last_poll_time = current_time

        # Security: Limit number of nodes polled at once
        node_ids = list(self._subscribed_nodes.keys())[: self.MAX_NODES_PER_POLL]
        if len(self._subscribed_nodes) > self.MAX_NODES_PER_POLL:
            logger.warning(
                f"Limiting poll to {self.MAX_NODES_PER_POLL} nodes (total subscribed: {len(self._subscribed_nodes)})",
            )
            ...
            for child in root_node.get_children():
                try:
                    ...
                    if child.get_node_class() == ua.NodeClass.Variable:
                        try:
                            node_info["data_type"] = str(
                                child.get_data_type_as_variant_type(),
                            )
