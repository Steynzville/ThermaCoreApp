...
def mqtt_publish():
    ...
    try:
        ...
        if not data or "topic" not in data or "payload" not in data:
            return SecurityAwareErrorHandler.handle_validation_error(
                ValueError("Topic and payload are required"),
                "MQTT publish",
            )
...
def opcua_read_node():
    ...
    try:
        ...
        if not data or "node_id" not in data:
            return SecurityAwareErrorHandler.handle_validation_error(
                ValueError("node_id is required"),
                "OPC UA node reading",
            )
...
def inject_test_scenario():
    ...
    try:
        ...
        if not data or "scenario_type" not in data:
            return SecurityAwareErrorHandler.handle_validation_error(
                ValueError("scenario_type is required"),
                "Scenario injection",
            )
