...
def add_alert_rule():
    ...
    try:
        ...
        return SecurityAwareErrorHandler.handle_validation_error(
            e, "Alert rule threshold parsing"
        )
    except Exception as e:
        return SecurityAwareErrorHandler.handle_service_error(
            e, "internal_error", "Alert rule creation", 500
        )
    ...
    try:
        ...
        if not current_app.protocol_simulator.connected:
            if not current_app.protocol_simulator.connect_mqtt():
                return SecurityAwareErrorHandler.handle_service_error(
                    Exception("MQTT connection failed"),
                    "connection_error",
                    "Simulator MQTT connection",
                    500,
                )
        ...
        return SecurityAwareErrorHandler.handle_service_error(
            e, "internal_error", "Simulator start", 500
        )
...
def inject_test_scenario():
    ...
    try:
        ...
        current_app.protocol_simulator.inject_test_scenario(
            scenario_type=data["scenario_type"], unit_id=data.get("unit_id")
        )

        return jsonify(
            {"status": "scenario_injected", "scenario_type": data["scenario_type"]}
        )

    except Exception as e:
        return SecurityAwareErrorHandler.handle_service_error(
            e, "internal_error", "Scenario injection", 500
        )
...
def get_all_devices_status():
    ...
    online_devices = sum(
        1 for device in devices_status if device.get("connected", False)
    )
