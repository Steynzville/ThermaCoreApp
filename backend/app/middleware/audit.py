...
def redact_sensitive_data(data, sensitive_keys=None, sensitive_patterns=None):
    ...
    if isinstance(data, list):
        return [
            redact_sensitive_data(item, sensitive_keys, sensitive_patterns)
            for item in data
        ]
...
class AuditLogger:
    ...
    def log_event(
        ...
        try:
            ...
            request_id = (
                RequestIDManager.get_request_id() if has_request_context() else None
            )
            ...
            if not user_id and not username and has_request_context():
                try:
                    ...
                    logger.warning(
                        f"Failed to extract user info from JWT: {e.__class__.__name__}",
                    )
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                logger.warning(
                    f"Failed to extract user info in audit decorator: {e.__class__.__name__}",
                )
            ...
            if include_request_data and has_request_context() and request:
                if request.is_json:
                    try:
                        ...
                        details["request_data"] = redact_sensitive_data(
                            raw_request_data,
                        )
                    except Exception:
            ...
            resource_id = (
                kwargs.get("id") or kwargs.get("unit_id") or kwargs.get("user_id")
            )
            ...
            try:
                ...
                if include_response_data and hasattr(result, "get_json"):
                    try:
                        ...
                        logger.debug(
                            f"Failed to extract response data: {e.__class__.__name__}",
                        )
...
def setup_audit_middleware(app):
    ...
    def audit_api_access():
        ...
        if request.endpoint and not request.endpoint.startswith("static"):
            ...
            redacted_query_params = redact_sensitive_data(
