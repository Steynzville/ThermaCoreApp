    ...
    def decorator(f: Callable) -> Callable:
        ...
        def decorated_function(*args, **kwargs):
            ...
            if key_func:
                ...
                identifier = (
                    f"endpoint:{request.endpoint}:{request.remote_addr or 'unknown'}"
                )
            else:
            ...
            if isinstance(response, tuple):
                # Response can be a tuple of response_obj with status_code and optional headers
            ...
            if response_obj and hasattr(response_obj, "headers"):
                ...
                response_obj.headers["X-RateLimit-Window"] = str(
                    rate_info["window_seconds"],
                )
...
class RateLimitConfig:
    ...
    # General API limits
    STANDARD: ClassVar[dict[str, int]] = {
        "limit": 100,
        "window_seconds": 60,
    }  # 100 requests per minute
    STRICT: ClassVar[dict[str, int]] = {
        "limit": 30,
        "window_seconds": 60,
    }  # 30 requests per minute
    RELAXED: ClassVar[dict[str, int]] = {
        "limit": 300,
        "window_seconds": 60,
    }  # 300 requests per minute

    # Specific endpoint limits
    AUTH_ENDPOINT: ClassVar[dict[str, int]] = {
        "limit": 10,
        "window_seconds": 60,
    }  # Login attempts
    SEARCH_ENDPOINT: ClassVar[dict[str, int]] = {
        "limit": 50,
        "window_seconds": 60,
    }  # Search queries
    DATA_UPLOAD: ClassVar[dict[str, int]] = {
        "limit": 20,
        "window_seconds": 60,
    }  # Data uploads

    # User-specific limits
    FREE_TIER: ClassVar[dict[str, int]] = {
        "limit": 1000,
        "window_seconds": 3600,
    }  # 1000 per hour
    PREMIUM_TIER: ClassVar[dict[str, int]] = {
        "limit": 10000,
        "window_seconds": 3600,
    }  # 10000 per hour
