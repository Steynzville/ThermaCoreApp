    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            summary["top_endpoints"] = sorted(
                endpoint_stats, key=lambda x: x["calls"], reverse=True
            )[:10]
                ...
                result.update(
                    {
                        "p50_response_time": round(
                            self._percentile(sorted_times, 50), 4
                        ),
                        "p95_response_time": round(
                            self._percentile(sorted_times, 95), 4
                        ),
                        "p99_response_time": round(
                            self._percentile(sorted_times, 99), 4
                        ),
                    }
                )
        ...
        if actual_exc is not None:
            ...
            if response.status_code >= 400:
                ...
                synthetic_exc = SyntheticHTTPException(
                    error_msg, response.status_code, exc_class_name
                )
