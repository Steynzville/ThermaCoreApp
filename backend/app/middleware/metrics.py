...
class MetricsCollector:
    ...
    def record_request_end(self, status_code: int, error: Exception | None = None):
                ...
                result.update(
                    {
                        "p50_response_time": round(
                            self._percentile(sorted_times, 50),
                            4,
                        ),
                        "p95_response_time": round(
                            self._percentile(sorted_times, 95),
                            4,
                        ),
                        "p99_response_time": round(
                            self._percentile(sorted_times, 99),
                            4,
                        ),
                    },
                )
...
def setup_metrics_middleware(app):
    ...
    if hasattr(app, "metrics_middleware_setup"):
        return app
...
def create_metrics_blueprint():
    ...
    from flask import Blueprint  # noqa: PLC0415 - Blueprint factory pattern
