...
def test_update_user_scenarios(client, admin_token):
    ...
    with patch("app.models.User.query") as mock_user_query, \
        ...
        with patch("app.routes.users.get_current_user_id", return_value=(2, True)):
            response = client.put("/api/v1/users/2", json={"role_id": 1}, headers=headers)
        ...
        response = client.put("/api/v1/users/2", json={"first_name": "DupUser"}, headers=headers)
        ...
        response = client.put("/api/v1/users/2", json={"first_name": "DupEmail"}, headers=headers)
        assert response.status_code == 409
