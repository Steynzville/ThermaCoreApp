...
class TestSensorsAPI:
    ...
    def test_create_unit_sensor_success(self, client, admin_token, db_session):
        ...
        response = client.post("/api/v1/units/TEST001/sensors", json=payload, headers=headers)
        ...
        # Cleanup
        sensor = Sensor.query.filter_by(unit_id="TEST001", name="Flow Rate Sensor").first()
        if sensor:
    ...
    def test_create_unit_sensor_unit_not_found(self, client, admin_token):
        ...
        response = client.post("/api/v1/units/NONEXISTENT/sensors", json=payload, headers=headers)
    ...
    def test_get_unit_readings_time_series(self, client, admin_token, db_session):
        ...
        try:
            ...
            response = client.get("/api/v1/units/TEST001/readings?hours=2", headers=headers)
            assert response.status_code == 200
            data = json.loads(response.data)
            assert isinstance(data, list)
