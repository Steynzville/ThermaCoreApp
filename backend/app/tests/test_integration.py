...
class TestIntegrationWorkflows:
    ...
    def test_complete_unit_lifecycle(self, client, db_session):
        ...
        sensor_response = client.post(
            "/api/v1/units/LIFECYCLE001/sensors",
            json=sensor_data,
            headers={
                "Authorization": f"Bearer {token}",
                "Content-Type": "application/json",
        ...
        update_response = client.patch(
            "/api/v1/units/LIFECYCLE001/status",
            json=status_update,
            headers={
                "Authorization": f"Bearer {token}",
                "Content-Type": "application/json",
        ...
        get_response = client.get(
            "/api/v1/units/LIFECYCLE001", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_user_role_workflow(self, client, db_session):
        ...
        units_response = client.get(
            "/api/v1/units", headers={"Authorization": f"Bearer {operator_token}"}
        )
    ...
    def test_data_filtering_and_pagination(self, client, db_session):
        ...
        online_units = client.get(
            "/api/v1/units?status=online", headers={"Authorization": f"Bearer {token}"}
        )
        ...
        critical_units = client.get(
            "/api/v1/units?health_status=critical",
        ...
        site_a_units = client.get(
            "/api/v1/units?location=Site A",
            headers={"Authorization": f"Bearer {token}"},
        )
        ...
        search_units = client.get(
            "/api/v1/units?search=Maintenance",
            headers={"Authorization": f"Bearer {token}"},
        )
        ...
        # 5. Test pagination
        page1 = client.get(
            "/api/v1/units?page=1&per_page=2",
            headers={"Authorization": f"Bearer {token}"},
        )
    ...
    def test_api_error_handling(self, client):
        ...
        assert response.status_code in [
            400,
            201,
        ]  # May pass validation but fail business rules
        ...
        response = client.get(
            "/api/v1/units", headers={"Authorization": "Bearer invalid_token"}
        )
    ...
    def test_database_consistency(self, client, db_session):
        ...
        for sensor_type in sensor_types:
            ...
            sensor_response = client.post(
                "/api/v1/units/CONSISTENCY001/sensors",
                json=sensor_data,
                headers={
                    "Authorization": f"Bearer {token}",
                    "Content-Type": "application/json",
        ...
        unit_response = client.get(
            "/api/v1/units/CONSISTENCY001", headers={"Authorization": f"Bearer {token}"}
        )
