...
class TestSCADAIntegration:
    ...
    def test_mqtt_to_database_pipeline(self, app):
        ...
        with app.app_context():
            ...
            sensor = Sensor.query.filter_by(
                unit_id="UNIT001", sensor_type="temperature"
            ).first()
    ...
    def test_alert_system(self, app):
        ...
        with app.app_context():
            ...
            alerts = realtime_processor._check_alert_rules(
                "UNIT001", "temperature", high_temp_data
            )
            assert len(alerts) == 1
            ...
            alerts = realtime_processor._check_alert_rules(
                "UNIT001", "temperature", normal_temp_data
            )
            assert len(alerts) == 0
    ...
    def test_service_integration_in_app(self, app):
        ...
        with patch.dict(
            os.environ,
            {"FLASK_ENV": "development", "FLASK_DEBUG": "1", "TESTING": "false"},
            clear=False,
        ):
