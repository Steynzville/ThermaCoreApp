...
class TestAuditDecorators:
    ...
    def test_audit_login_success_function(self):
        ...
        with patch(
            "app.middleware.audit.AuditLogger.log_authentication_event",
        ) as mock_audit:
    ...
    def test_audit_permission_check_function(self):
        ...
        with patch(
            "app.middleware.audit.AuditLogger.log_authorization_event",
        ) as mock_audit:
...
class TestAuditMiddleware:
    ...
    def test_setup_audit_middleware(self, app):
        ...
        with app.app_context():
            # The middleware should already be set up during app creation
            # Just verify before_request handlers are registered
            assert len(app.before_request_funcs[None]) > 0

            # Verify the audit middleware is present by checking for typical audit behavior
            # The middleware sets up request tracking, so we can verify that works
