...
class TestSecureLoggingIntegration:
    ...
    def test_multiple_sensitive_patterns_sanitized(self, app):
        ...
        with app.app_context():
            error = Exception(
                "Auth failed: password=secret123 token=xyz789 api_key=abc456",
            )
    ...
    def test_error_context_preserved_in_logs(self, app):
        ...
        with app.app_context():
            error = Exception(
                "User admin login failed with password=secret at /api/v1/login",
            )
            ...
            with patch("app.utils.error_handler.logger") as mock_logger:
                ...
                assert (
                    "user_login" in log_message
                    or call_args[1].get("extra", {}).get("context") == "user_login"
                )
