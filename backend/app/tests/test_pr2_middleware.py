...
class TestRequestValidator:
    ...
    def test_validate_request_size_success(self, app):
        ...
        with app.test_request_context("/", method="POST", content_length=500):
    ...
    def test_validate_request_size_failure(self, app):
        ...
        with app.test_request_context("/", method="POST", content_length=2000):
            g.request_id = "test-id"
            result = RequestValidator.validate_request_size(1000)
            assert result is not None
            response, status_code = result
...
class TestMetricsCollector:
    ...
    def test_metrics_middleware_integration(self, app):
        ...
        with app.test_client() as client:
            ...
            assert any("GET" in key for key in collector.request_count)
    ...
    def test_get_endpoint_metrics_xss_protection(self, app):
        ...
        with app.test_request_context(malicious_endpoint, method="GET"):
            ...
            assert "&gt;" in metrics["endpoint"]  # > should be escaped to &gt;
            assert (
                "<script>" not in metrics["endpoint"]
            )  # Raw script tag should not be present
            assert (
                "</script>" not in metrics["endpoint"]
            )  # Raw closing script tag should not be present
            assert (
                "alert" in metrics["endpoint"]
            )  # But the text content should still be there (escaped)
            assert "&#34;" in metrics["endpoint"]  # " should be escaped to &#34;
    ...
    def test_get_metrics_summary_xss_protection(self, app):
        ...
        with app.test_request_context("/", method="GET"):
            ...
            for endpoint_key in summary["endpoints"]:
            ...
            for endpoint_key in summary["error_summary"]["error_rate_by_endpoint"]:
                # Dangerous opening tags should not be present in executable form
                assert "<script>" not in endpoint_key
                assert "</script>" not in endpoint_key
                assert "<img" not in endpoint_key
...
class TestSecurityAwareErrorHandler:
    ...
    def test_handle_service_error(self, app):
        ...
        with app.test_request_context("/"):
            ...
            assert (
                data["error"]["message"]
                == SecurityAwareErrorHandler.GENERIC_MESSAGES["validation_error"]
            )
