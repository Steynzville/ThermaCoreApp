        ...
        with patch("app.services.opcua_service.opcua_available", True):
            with patch("app.services.opcua_service.Client"):
                with patch.dict(
                    ...
                    with pytest.raises(
                        ValueError,
                        match="requires client certificates.*Set OPCUA_ALLOW_INSECURE_FALLBACK=true",
                    ):
        ...
        with patch("app.services.opcua_service.opcua_available", True):
            with patch("app.services.opcua_service.Client"):
                with patch.dict(
                    ...
                    with pytest.raises(
                        ValueError, match="requires client certificates in production"
                    ):
                        opcua_client.init_app(mock_app)
