...
class TestEnvironmentMismatchDetection:
    ...
    def test_production_environment_raises_on_debug_mismatch(self):
        ...
        with patch.dict(os.environ, {"FLASK_ENV": "production", "TESTING": "false"}):
            with pytest.raises(
                ValueError, match="Dangerous environment mismatch detected"
            ):
...
class TestOPCUACertificateLoading:
    ...
    def create_test_certificate(self) -> Path:
        ...
        subject = issuer = x509.Name(
            [
                ...
                x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Test OPC UA"),
                x509.NameAttribute(NameOID.COMMON_NAME, "test.opcua.server"),
            ]
        )
        ...
        certificate = (
            ...
            .add_extension(
                x509.SubjectAlternativeName(
                    [
                        x509.DNSName("test.opcua.server"),
                    ]
                ),
                critical=False,
            )
    ...
    def create_expired_certificate(self) -> Path:
        ...
        subject = issuer = x509.Name(
            [
                x509.NameAttribute(NameOID.COMMON_NAME, "expired.opcua.server"),
            ]
        )
    ...
    def test_certificate_loading_success(self):
        ...
        try:
            with patch.dict(
                os.environ,
                {"FLASK_ENV": "development", "TESTING": "false"},
                clear=False,
            ):
                mock_app = Mock()
                mock_app.config = {
                ...
                with patch("app.services.opcua_service.opcua_available", True):
                    with patch(
                        "app.services.opcua_service.Client"
    ...
    def test_certificate_loading_file_not_found(self):
        ...
        with patch.dict(
            os.environ, {"FLASK_ENV": "development", "TESTING": "false"}, clear=False
        ):
            mock_app = Mock()
            mock_app.config = {
                "FLASK_ENV": "development",
            ...
            with patch("app.services.opcua_service.opcua_available", True):
                with patch("app.services.opcua_service.Client"):
                    ...
                    with pytest.raises(
                        ValueError, match="OPC UA trust certificate file does not exist"
                    ):
                        opcua_client.init_app(mock_app)
            ...
            with patch.dict(
                ...
                with patch("app.services.opcua_service.opcua_available", True):
                    with patch("app.services.opcua_service.Client"):
                        ...
                        with pytest.raises(
                            ValueError, match="Server certificate has expired"
                        ):
                            opcua_client.init_app(mock_app)
            ...
            with patch.dict(
                ...
                with patch("app.services.opcua_service.opcua_available", True):
                    with patch("app.services.opcua_service.Client"):
                        ...
                        with pytest.raises(
                            ValueError, match="Invalid certificate format"
                        ):
                            opcua_client.init_app(mock_app)
    ...
    def test_production_requires_trust_certificate(self):
        ...
        with patch.dict(
            os.environ, {"FLASK_ENV": "production", "TESTING": "false"}, clear=False
        ):
            mock_app = Mock()
            mock_app.config = {
            ...
            with patch("app.services.opcua_service.opcua_available", True):
                with patch("app.services.opcua_service.Client"):
                    with patch("app.services.opcua_service.logger") as mock_logger:
                        ...
                        assert (
                            result is None
                        )  # init_app returns None when it returns early
...
class TestImprovedExceptionHandling:
    ...
    def test_service_initialization_reraises_in_production(self):
        ...
        with patch.dict(os.environ, {"FLASK_ENV": "production", "TESTING": "false"}):
            with pytest.raises(
                RuntimeError, match="security validation failed in production"
            ):
                _initialize_critical_service(
                    mock_service, "Test Service", mock_app, mock_logger
                )
