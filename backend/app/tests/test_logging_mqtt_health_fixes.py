    ...
    def test_mqtt_initialization_logging_success(self, mock_mqtt_client, mock_app):
        ...
        with patch("app.services.mqtt_service.logger") as mock_logger:
            ...
            mock_logger.info.assert_any_call(
                "MQTT configuration loaded - Broker: localhost:1883, TLS: False",
            )
            ...
            mock_logger.info.assert_any_call(
                "MQTT service initialization completed successfully",
            )
            ...
            with patch(
                ...
                with patch("os.path.exists", return_value=True):
                    with patch("os.path.getsize", return_value=100):
                        ...
                        # Verify error logging - check that exception was called with the message
                        exception_calls = [
                            str(call) for call in mock_logger.exception.call_args_list
                        ]
                        assert any(
                            "Error configuring MQTT TLS" in str(call)
                            for call in exception_calls
                        )

    @patch("app.services.mqtt_service.mqtt.Client")
    def test_mqtt_initialization_callback_error_raises(
        self,
        mock_mqtt_client,
        mock_app,
    ):
