...
class TestSecurityHardeningImprovements:
    ...
    def test_string_sanitization_in_loop(self):
        ...
        try:
            ...
            # Expected to fail due to DB context
            assert e is not None  # Just verify exception occurred
    ...
    @pytest.mark.skip(
        reason="MQTT production security enforcement not yet implemented - aspirational test",
    )
    def test_mqtt_production_security_enforcement(self):
        ...
        with pytest.raises(
            ValueError,
            match="MQTT TLS must be enabled in production environment",
        ):
            mqtt_client.init_app(mock_app, None)

    @pytest.mark.skip(
        reason="MQTT authentication enforcement not yet implemented - aspirational test",
    )
    def test_mqtt_production_authentication_enforcement(self):
        ...
        with pytest.raises(
            ValueError,
            match="MQTT authentication must be configured in production environment",
        ):
            mqtt_client.init_app(mock_app, None)
    ...
    @pytest.mark.skip(
        reason="OPC UA authentication enforcement not yet implemented - aspirational test",
    )
    def test_opcua_production_security_enforcement(self):
        ...
        with pytest.raises(
            ValueError,
            match="OPC UA authentication must be configured in production environment",
        ):
    ...
    def test_opcua_weak_security_policy_enforcement(self):
        ...
        with patch("app.services.opcua_service.opcua_available", True):
            with patch("app.services.opcua_service.Client"):
                ...
                with pytest.raises(
                    ValueError,
                    match="OPC UA security policy .* requires client certificates",
                ):
                    opcua_client.init_app(mock_app, None)
