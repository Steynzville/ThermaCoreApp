...
def test_initialize_opcua_clients_all_fail():
    ...
    with patch("app.service_init.safe_service_init") as mock_safe_init:
        ...
        mock_safe_init.side_effect = [
            Exception("Secure failed"),
            Exception("Standard failed"),
        ]
...
@patch(
    "app.service_init.should_skip_external_services",
    side_effect=Exception("Crash during skip check"),
)
