...
class TestSecurityImprovements:
    ...
    def test_service_unavailable_handling(self, app):
        ...
        with app.app_context():
            response, status_code = (
                SecurityAwareErrorHandler.handle_service_unavailable("MQTT client")
            )
            ...
            assert (
                response_data["error"]["message"] == "Service is currently unavailable."
            )
    ...
    def test_data_storage_race_condition_handling(self, app):
        ...
        with app.app_context():
            with (
                ...
                result = data_storage_service.find_or_create_sensor(
                    "UNIT001", "temperature"
                )
    ...
    def test_data_storage_unit_not_found(self, app):
        ...
        with app.app_context():
            with (
                patch("app.services.data_storage_service.db"),
                patch("app.services.data_storage_service.Unit") as mock_unit,
            ):
                # Unit doesn't exist
