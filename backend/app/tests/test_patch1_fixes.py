...
class TestMQTTErrorCaching:
    ...
    def test_mqtt_error_logged_once(self, app, client):
        ...
        with app.app_context():
            ...
            if hasattr(app, "mqtt_error_logged"):
                delattr(app, "mqtt_error_logged")
    ...
    def test_mqtt_error_flag_set(self, app, client):
        """Test that mqtt_error_logged flag is set after error."""
        with app.app_context():
            # Mock MQTT client that raises exception
            mock_client = Mock()
            mock_client.get_status.side_effect = Exception("MQTT connection failed")
            ...
            assert not hasattr(app, "mqtt_error_logged")
            ...
            assert hasattr(app, "mqtt_error_logged")
            assert app.mqtt_error_logged is True
    ...
    def test_mqtt_error_returns_correct_status(self, app, client):
        """Test that detailed health check returns correct status on MQTT error."""
        with app.app_context():
            # Mock MQTT client that raises exception
            mock_client = Mock()
            ...
            # Detailed endpoint always returns healthy
            assert data["status"] == "healthy"

            # Services field should exist in detailed endpoint
