...
def verify_database_schema():
    ...
    for column_name, expected_spec in REQUIRED_COLUMNS.items():
        if column_name not in actual_columns:
            ...
            if expected_type in col_type_str or expected_type.replace('VARCHAR', 'STRING') in col_type_str:
                ...
                print(f"⚠ Column '{column_name}' exists but type mismatch: expected {expected_type}, got {col_type_str}")
    ...
    if not missing_columns and not mismatched_columns and not missing_indexes:
        ...
        if mismatched_columns:
            ...
            for col in mismatched_columns:
                print(f"   - {col['name']}: expected {col['expected']}, got {col['actual']}")
