    ...
    try:
        with app.app_context():
            ...
            if len(permissions) == 0:
                ...
                print_info("Run seed data script: psql $DATABASE_URL -f migrations/002_seed_data.sql")
                return False
            else:
            ...
            if len(roles) == 0:
                ...
                for role in roles:
                    ...
                    if perm_count == 0:
                        print_warning(f"    Role '{role.name.value}' has NO permissions assigned!")
            ...
            if not admin_role:
                ...
                if len(admin_role.permissions) < 8:
                    print_warning(f"Admin role has only {len(admin_role.permissions)} permissions (expected 8)")
                    print_info("Admin should have all permissions. Re-run seed data script.")
        ...
        with app.app_context():
            ...
            if len(admin_users) == 0:
                ...
                for user in admin_users:
                    ...
                    if not user.role:
                        ...
                        print_success(f"  - Role has {len(user.role.permissions)} permissions")
            ...
            if steyn_admin:
                ...
                if not steyn_admin.role:
                    ...
                    print_info("  - This WILL cause 500 error: user.role.name.value will fail!")
                    return False
            else:
    ...
    try:
        with app.app_context():
            ...
            if admin_user.role:
                ...
                print_error("     The line 'user.role.name.value' will fail with AttributeError")
                checks.append(False)
