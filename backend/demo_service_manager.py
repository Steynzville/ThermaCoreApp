...
def demo_service_classification():
    ...
    for name, status in manager.get_all_services_status().items():
        print(f"  • {name:15} - Type: {status['type']:8} | Enabled: {status['enabled']} | Status: {status['status']}")
...
def demo_graceful_degradation():
    ...
    for name, status in manager.get_all_services_status().items():
        ...
        print(f"  {symbol} {name:10} - {status['status']:15} | Available: {status['available']}")
        if status['error']:
...
def demo_configuration():
    ...
    for env_name, config in configs:
        ...
        for key, value in config.items():
            service = key.replace('SERVICE_', '').replace('_ENABLED', '').replace('_REQUIRED', '')
            if '_ENABLED' in key:
