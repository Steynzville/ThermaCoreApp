...
class APITester:
    ...
    def test_health_endpoint(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.print_warning(f"Health endpoint returned unexpected status: {response.status_code}")
                print("\nResponse:")
                print(response.text)
                return False
                
        except RequestException as e:
            self.print_error("Failed to connect to backend - network error or server is down")
    ...
    def test_login_endpoint(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.print_warning(f"Login endpoint returned unexpected status: {response.status_code}")
                print("\nResponse:")
                try:
                    print(self.format_json(response.json()))
                except:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    self.print_error("Failed to connect to alternative dashboard endpoint")
                    print(f"\nError details: {e}")
                    return False
                ...
                self.print_warning(f"Dashboard endpoint returned unexpected status: {response.status_code}")
                print("\nResponse:")
                try:
                    print(self.format_json(response.json()))
                except:
    ...
    def print_summary(self):
        ...
        if self.results['health']:
            ...
            self.print_error("Backend health check failed or returned unexpected status")
        ...
        if self.results['login'] and not self.results['dashboard']:
            ...
            print("   → Check dashboard endpoint exists and user has correct permissions")
...
def main():
    ...
    # Parse arguments
    if len(sys.argv) < 2:
        print("Usage: python backend/diagnose_api_endpoints.py [base_url] [username] [password]")
        print("\nExamples:")
        print("  python backend/diagnose_api_endpoints.py https://thermacoreapp.onrender.com admin YOUR_PASSWORD")
        print("  python backend/diagnose_api_endpoints.py http://localhost:5000 admin admin123")
        sys.exit(1)
